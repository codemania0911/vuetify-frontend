  ...
  >
    <v-row>
      ...
      >
        ...
        >
          <v-progress-linear
            v-if="networkLoading || loadingMixins.networks"
            indeterminate
          />
          ...
          <v-simple-table>
            ...
            <tbody v-if="!loadingMixins.networks">
              <tr
                v-for="network in mixinItems.networks"
                :key="network.id"
              >
                <td class="text-left">
                  {{ network.name }}
        ...
        >
          <v-progress-linear
            v-if="fleetLoading || loadingMixins.fleets"
            indeterminate
          />
          <v-divider class="mt-3 mb-5" />
          <v-simple-table>
            ...
            <tbody>
              <tr
                v-for="fleet in mixinItems.fleets"
                :key="fleet.id"
              >
                <td class="text-left">
                  {{ fleet.name }}
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addContinuous () {
        ...
        axios.post('ais/getaisbulkpositioncont', this.request)
          .then(res => {
            ...
            this.request.network ? this.getInitials([{ url: 'networks/short', item: 'networks' }]) : this.getInitials([{ url: 'fleets', item: 'fleets' }])
            this.getContVessels()
            this.deleteAll()
          })
      },
      ...
      deleteAll () {
        this.networkId ? this.getInitials([{ url: 'networks/short', item: 'networks' }]) : this.getInitials([{ url: 'fleets', item: 'fleets' }])
        this.networkId = 0
        this.fleetId = 0
