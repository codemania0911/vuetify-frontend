        ...
        <v-row v-if="aisdata.position">
          ...
          >
            <v-text-field
              ...
              v-model="(mixinItems.countries.find(c =>
  ...
  export default {
      ...
      aisdata (val) {
        ...
        if (val.position) {
          if (val.position.zone_id) {
            for (let i = 0; i < this.mixinItems.zones.length; i++) {
              if (val.position.zone_id === this.mixinItems.zones[i].id) {
                this.zone = this.mixinItems.zones[i].name
              }
            }
    ...
    methods: {
      getZoneName (zoneId) {
        ...
        return zoneId
          ? (this.mixinItems.zones.find(zone => zone.id === zoneId) || {}).name || outside
          : outside
      },
