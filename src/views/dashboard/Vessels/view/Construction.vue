              ...
              <v-row>
                ...
                >
                  <v-switch
                    ...
                    label="Lead Ship"
                    :disabled="!role || role && !isInternal(role.id)"
                    @change="saveLead"
                  />
                </v-col>
                <v-col
                ...
                >
                  <v-autocomplete
                    ...
                    :loading="loadingMixins.providers || savingProvider"
                    item-text="name"
                    item-value="id"
                    multiple
                    clearable
                ...
                >
                  <v-autocomplete
                    v-model="editedItem.vessel_class_id"
                    :items="mixinItems.vesselClasses"
                    item-text="name"
                    item-value="id"
                    clearable
                    :loading="savingVesselClass || loadingMixins.vesselClasses"
                    :disabled="!role || role && !isInternal(role.id)"
                    prepend-icon="mdi-source-repository-multiple"
                    label="Vessel Class"
                    @change="saveVesselClass"
                  />
                </v-col>
              </v-row>
            </v-tab-item>
  ...
  export default {
    ...
    methods: {
      ...
      async saveVesselClass () {
        ...
        this.savingVesselClass = true
        try {
          const response = await axios.post('vessels/update-vessel-class/' + this.$route.params.id, { vessel_class_id: this.editedItem.vessel_class_id })
          this.showSnackBar({ text: response.data.message, color: 'success' })
        } catch (error) {
          this.showSnackBar({ text: error, color: 'error' })
        }
        this.savingVesselClass = false
      },
    },
  }
</script>
